...
(function() {
    ...
    function exportFindings(kind) {
        ...
        if (kind === 'json') {
            ...
            const sarif = {
                ...
                version: '2.1.0',
                runs: [{
                    tool: { driver: { name: 'ADB Auditor', version: '2.0', informationUri: 'https://adbauditor.com', rules: [...rules.values()] } },
                    results,
    ...
    setupTheme();
    if (/[?&]debug=1\b/.test(location.search)) installDiagPanel();
    function installDiagPanel() {
        const wrap = document.createElement('div');
        wrap.id = 'diagPanel';
        wrap.innerHTML = '<div class="diag-head"><strong>Diagnostics</strong>' +
            '<span class="diag-count">0 events</span>' +
            '<button class="diag-btn" data-diag="copy" type="button">Copy</button>' +
            '<button class="diag-btn" data-diag="clear" type="button">Clear</button>' +
            '<button class="diag-btn" data-diag="hide" type="button">Hide</button>' +
        '</div><pre class="diag-log mono"></pre>';
        document.body.appendChild(wrap);
        const pre = wrap.querySelector('.diag-log');
        const count = wrap.querySelector('.diag-count');
        const refresh = () => {
            const entries = (window.ADBDiagnostics && window.ADBDiagnostics.entries()) || [];
            pre.textContent = entries.map(e => '[' + new Date(e.t).toISOString().slice(11, 23) + '] ' + (e.level === 'err' ? 'ERR ' : '') + e.msg).join('\n');
            count.textContent = entries.length + ' events';
            pre.scrollTop = pre.scrollHeight;
        };
        window.addEventListener('adb-diag', refresh);
        wrap.addEventListener('click', async (e) => {
            const a = e.target.closest('[data-diag]');
            if (!a) return;
            const action = a.getAttribute('data-diag');
            if (action === 'copy') {
                try { await navigator.clipboard.writeText(pre.textContent); a.textContent = 'Copied'; setTimeout(() => { a.textContent = 'Copy'; }, 1000); }
                catch (_) { showToast('Copy failed', 'error'); }
            } else if (action === 'clear') {
                if (window.ADBDiagnostics) window.ADBDiagnostics.clear();
                refresh();
            } else if (action === 'hide') {
        ...
        refresh();
        const diagLog = (msg) => {
            const evt = new CustomEvent('adb-diag', { detail: { t: Date.now(), level: 'log', msg } });
            const sink = window.ADBDiagnostics && window.ADBDiagnostics.entries();
            if (sink !== undefined) {
                try { window.ADBDiagnostics.entries().push({ t: Date.now(), level: 'log', msg }); } catch (_) {}
            }
            window.dispatchEvent(evt);
        };
        diagLog('[debug=1] panel ready. WebUSB available: ' + !!navigator.usb);
        if (navigator.usb && navigator.usb.getDevices) {
            navigator.usb.getDevices()
                .then(ds => diagLog('[debug=1] previously granted devices: ' + ds.length + ' [' + ds.map(d => d.productName || (d.vendorId + ':' + d.productId)).join(', ') + ']'))
                .catch(e => diagLog('[debug=1] getDevices error: ' + e.message));
        }
    }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (viewBtn) { window.viewStorageFile(viewBtn.dataset.path); return; }
        const dlBtn = e.target.closest('.storage-dl-btn');
        if (dlBtn) { window.downloadStorageFile(dlBtn.dataset.pkg, dlBtn.dataset.path, dlBtn.dataset.name); return; }
    });
