...
(function(global) {
    ...
    const _diagSink = [];
    function log(...args) {
        if (DEBUG) console.log('[ADB]', ...args);
        _diagSink.push({ t: Date.now(), level: 'log', msg: args.map(a => typeof a === 'string' ? a : safeStr(a)).join(' ') });
        if (_diagSink.length > 500) _diagSink.shift();
        emitDiag();
    }
    function logError(...args) {
        console.error('[ADB ERROR]', ...args);
        _diagSink.push({ t: Date.now(), level: 'err', msg: args.map(a => typeof a === 'string' ? a : safeStr(a)).join(' ') });
        if (_diagSink.length > 500) _diagSink.shift();
        emitDiag();
    }
    function safeStr(v) { try { return JSON.stringify(v); } catch (_) { return String(v); } }
    function emitDiag() {
        try {
            const evt = new CustomEvent('adb-diag', { detail: _diagSink[_diagSink.length - 1] });
            window.dispatchEvent(evt);
        } catch (_) {}
    }
    global.ADBDiagnostics = { entries: () => _diagSink.slice(), clear: () => { _diagSink.length = 0; } };
